...
int test_util::random_int() {
    int a = dist(mersenne_engine);
    return a;
}

// return a shuffleld vector containing values from 0 ... length - 1
std::vector<int> test_util::shuffled_vec_int(long unsigned int length) {
...
bool test_util::validate_ascend(std::vector<int> &input) {
...
void test_util::print_vec(std::vector<int> &input) {
    for (auto i : input) {
        std::cout << i << " ";
    }
    std::cout << "\n";
